...
function update_league_table(league_table) {
  ...
  $(league_table).each(function (key, value) {
    league_table_content += '<tr>' + '<td>' + value.team_name + '</td>' + '<td class="text-center">' + value.played + '</td>' + '<td class="text-center">' + value.won + '</td>' + '<td class="text-center">' + value.drawn + '</td>' + '<td class="text-center">' + value.lost + '</td>' + '<td class="text-center">' + value.gd + '</td>' + '<td class="text-center">' + value.points + '</td>' + '</tr>';
  });
  $('#league_table').html(league_table_content);
}

function update_match_results(matches_of_the_week) {
  var matches_of_the_week_content = '<table class="table table-striped table-bordered table-hover m-0">' + '<thead>' + '<tr>' + '<th colspan="3">' + matches_of_the_week[0].play_week + '. ' + week_match_results;
  '</th></tr>' + '</thead>';
  $(matches_of_the_week).each(function (key, value) {
    matches_of_the_week_content += '<tr>' + '<td>' + value.home_team.name + '</td>' + '<td>' + value.home_team_goal + ' - ' + value.away_team_goal + '</td>' + '<td>' + value.away_team.name + '</td>' + '</tr>';
  });
...
function update_champion_prediction(champion_prediction, week) {
  if (week >= 4 && week < 6) {
    ...
    $(champion_prediction).each(function (key, value) {
      champion_prediction_content += '<tr>' + '<td>' + value.name + '</td>' + '<td class="text-center"> %' + Math.round(value.points) + '</td>' + '</tr>';
    });
...
__webpack_require__(/*! C:\xampp\htdocs\league_simulation\resources\js\app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! C:\xampp\htdocs\league_simulation\resources\css\app.css */"./resources/css/app.css");
